...
cp --update=none ../static/placeholder.png ../img_all/'||regexp_replace(unaccent(coalesce(ptrl.name,f.name)), '[^\w]+','','g')||'-Report_BrERP_v11.0.0.png
        ...
        LEFT JOIN (
            SELECT pp.ad_process_id, 
                   string_agg(
                       ...
                       '<br/>' ||(SELECT coalesce(r.name,'') FROM ad_reference r WHERE r.validationtype='D' AND r.ad_reference_id=pp.ad_reference_id) ||
                       ' | ' || chr(10), ''
