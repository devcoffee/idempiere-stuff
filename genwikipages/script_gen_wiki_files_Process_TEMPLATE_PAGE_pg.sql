...
SELECT
'cat > ./process/'||regexp_replace(unaccent(coalesce(ptrl.name,f.name)), '[^\w]+','','g')||'_Process_ID-'||f.ad_process_id||'_v10.0.0.md <<!
...
![](/img/system-manual/brerp/'||regexp_replace(unaccent(coalesce(ptrl.name,f.name)), '[^\w]+','','g')||'-Process_BrERP_v10.0.0.png)
...
cp ../static/placeholder.png ../img_all/'||regexp_replace(unaccent(coalesce(ptrl.name,f.name)), '[^\w]+','','g')||'-Process_BrERP_v10.0.0.png
' AS wikitext
