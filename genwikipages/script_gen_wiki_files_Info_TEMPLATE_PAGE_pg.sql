...
SELECT
'cat > ./info/'||regexp_replace(unaccent(coalesce(itrl.name,f.name)), '[^\w]+','','g')||'_Info_ID-'||f.ad_infowindow_id||'_v11.0.0.md <<!
...
![](/img/system-manual/brerp/'||regexp_replace(unaccent(coalesce(itrl.name,f.name)), '[^\w]+','', 'g')||'-Info_BrERP_v11.0.0.png)

!

cp --update=none ../static/placeholder.png ../img_all/'||regexp_replace(unaccent(coalesce(itrl.name,f.name)), '[^\w]+','', 'g')||'-Info_BrERP_v11.0.0.png
